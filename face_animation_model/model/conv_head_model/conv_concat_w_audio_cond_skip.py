...
def get_infusion_block_with_norm(block_out, activation, padding_type, norm):
    ...
    infuse_block.append(nn.Conv1d(block_out, block_out, kernel_size=3, stride=1, padding=1,
                                  padding_mode=padding_type))
    add_activation(activation, infuse_block)
    add_norm(norm, block_out, infuse_block)
    return nn.Sequential(*infuse_block)

class Infusion_block_with_norm(nn.Module):
    def __init__(self, motion_feat_dim, condion_feat_dim, activation, padding_type, norm):
        super().__init__()

        infuse_block = []
        infuse_block.append(nn.Conv1d(motion_feat_dim+condion_feat_dim, motion_feat_dim, kernel_size=3, stride=1, padding=1,
                                    padding_mode=padding_type))
        add_activation(activation, infuse_block)
        add_norm(norm, motion_feat_dim, infuse_block)
        self.mle_layer = nn.Sequential(*infuse_block)
...
class Downsample_without_norm(nn.Module):
    def __init__(self, block_in, block_out, padding_type, activation):
        ...
        self.audio_linear_projection = nn.AvgPool1d(kernel_size=2, stride=2)

    def forward(self, input, audio):
        return self.mle_layer(input), self.audio_linear_projection(audio)
...
class simple_Enc_with_MLE_with_w_time_embedding(nn.Module):
    def __init__(self,
        ...
        self.in_infuse = Infusion_block_with_norm(ch, cond_feat_dim, activation, padding_type, norm)
        ...
        for i in range(len(ch_multi) - 1): # 2 -1 = 1 [1, 2]
            ...
            infusion_layers.append(Infusion_block_with_norm(block_out, cond_feat_dim, activation, padding_type, norm))
...
class decoder_2layer_block_w_audio_new_layers(nn.Module):
    def __init__(self,
        ...
        if final_activation is not None:
            ...
            self.out_embed,
            get_inplace_activation(final_activation),
            )

    def forward(self, x, audio_feats:list, skip:list, one_hot:torch.Tensor):
        """
        x : Bs x feat_dim x T
        one_hot: Bs x num_classes
        """
        Bs, audio_dim, T = x.shape
        audio_feats.pop() # remove the last one
        skip.pop() # remove the last one

        for up, infuse in zip(self.mle_layers, self.infusion_layers):
            # for up in self.mle_layers:
            x = up(x)
            # print("dec", x.shape, audio_feats[-1].shape)
            x = torch.cat([x, audio_feats.pop()], dim=1) # (Bs, 2*ch, T)
            x = infuse(x)
            x = x + skip.pop()
...
class in_to_motion_unet_w_audio_cond(nn.Module):
    ...
    def forward(self, disp, timesteps, **kwargs):
        ...
        batch = kwargs["batch"]
        if len(batch) == 5:
            audio_feat, motion, template, one_hot, _ = batch
        else:
            audio_feat, motion, template, one_hot, filename, gtpose = batch

        self.device = disp.device
        ...
        x, audio_conds, skip_conds = self.encoder(disp.permute(0, 2, 1), audio_feat.permute(0, 2, 1),  timesteps)
        all_disp_out = self.decoder(x, audio_conds, skip_conds, one_hot).permute(0, 2, 1)
...
################################################################################################
class in_to_motion_unet_w_audio_cond_with_cond_mask(in_to_motion_unet_w_audio_cond):
    def __init__(self, **args):
        super().__init__(**args)
...
################################################################################################
####### Final model  with masking and audio encoder ################
################################################################################################
class unet_w_audio_cond_mask_with_audio_encoding(in_to_motion_unet_w_audio_cond):
    def __init__(self, **args):
        super().__init__(**args)
        ## creating audio encoder
        self.create_audio_encoder(struct(**args))
    ...
    def forward(self, disp, timesteps, **kwargs):
        ...
        if len(batch) == 5:
            ...
            audio, motion, template, one_hot, filename, gtpose = batch
        ...
        # add : for the conditional model as unconditional debugging exp
        x, audio_cons, skip_cons = self.encoder(disp.permute(0, 2, 1), audio_feat.permute(0, 2, 1),  timesteps)
        all_disp_out = self.decoder(x, audio_cons, skip_cons, one_hot).permute(0, 2, 1)
