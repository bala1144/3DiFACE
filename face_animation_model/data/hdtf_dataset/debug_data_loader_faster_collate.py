...
class Dataset_v2(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        if self.data_type in ["train", "val"] and seq_len > self.window:
            audio = audio.view(-1, 1)
            possible_idxs = np.arange(0, seq_len - self.window)
            np.random.shuffle(possible_idxs)
            num_of_samples = min(len(possible_idxs), self.patches_per_seq)

            start_idxs = possible_idxs[:num_of_samples]
            crop_idx = start_idxs.reshape(-1, 1) + self.increments
            vertice = vertice[crop_idx, :]
            pose = pose[crop_idx, :]


            audio_start_idxs = np.int64(start_idxs / self.fps * self.audio_sample_rate)
            a_crop_idxs = audio_start_idxs.reshape(-1, 1) + self.a_increments
            audio = audio[a_crop_idxs.reshape(-1)].view(self.patches_per_seq, -1)

            template = template.view(1, -1).expand(num_of_samples, -1)   # need to patch times
            one_hot = one_hot.view(1, -1).expand(num_of_samples,-1)  # need to patch times
            file_name = [file_name.replace(".wav", "_start%03d.wav" % idx) for idx in
                         start_idxs]  # need to patch times
...
def read_data(
    ...
    for subj in all_subjects:
        ...
        if os.path.isfile(processed_audio_feat_file):
        ...
        key = subj
        data[key]["audio"] = input_values.reshape(-1)
        temp = templates[subj]
        data[key]["name"] = f
        ...
        data[key]["pose"] = pose_processing(sub_dict["global_pose"][:max_seq_len], normalize_pose,
                                            normalize_dim, to_6d_rotation, smooth_sigma)
        # ### extract the head pose
        # if normalize_pose:
        #     if normalize_dim == "xyz":
        #         pose = sub_dict["global_pose"]  # T x 3 # numpy
        #         euler_pose = Rotation.from_rotvec(pose).as_euler("xyz", degrees=True)
        #         simple_rot = euler_pose - euler_pose[:1]
        #         norm_pose_aa = Rotation.from_euler("xyz", simple_rot, degrees=True).as_rotvec()
        #         norm_pose_aa = norm_pose_aa.astype(np.float32).reshape(-1, 3)
        #         data[key]["pose"] = norm_pose_aa
        #     elif normalize_dim == "z"  or normalize_dim == "y":
        #         if normalize_dim=="y":
        #             order = "yzx"
        #         elif normalize_dim=="z":
        #             order = "zxy"
        #
        #         pose = sub_dict["global_pose"]  # T x 3 # numpy
        #         euler_pose = Rotation.from_rotvec(pose).as_euler(order, degrees=True)
        #         ref_frame = euler_pose[:1]
        #         norm_z = euler_pose[:, :1] - ref_frame[:, :1]
        #         norm_z_pose = np.concatenate([norm_z, euler_pose[:, 1:]], axis=1)
        #         norm_z_pose = Rotation.from_euler(order, norm_z_pose, degrees=True).as_rotvec()
        #         norm_z_pose = norm_z_pose.astype(np.float32).reshape(-1, 3)
        #         data[key]["pose"] = norm_z_pose
        # else:
        #     data[key]["pose"] = sub_dict["global_pose"]
        #
        # if to_6d_rotation:
        #     pose_6d = Rotation.from_rotvec(data[key]["pose"]).as_matrix().reshape(-1,9)[:, :6]
        #     data[key]["pose"] = pose_6d.astype(np.float32).reshape(-1, 6)
...
class DataModuleFromConfig_windows():
    def __init__(self, **kwargs):
        ...
        self.subjects_dict = subjects_dict
        train_data = self.process_seq(train_data, self.window, "train")
        valid_data = self.process_seq(valid_data, self.window, "val")
        test_data = self.process_seq(test_data, self.window, "test")
    ...
    def process_seq(self, data:list, window:int, dataset:str):

        one_hot_labels = np.eye(self.num_iden_cls)
        possible_test_subjects = self.subjects_dict["train_subjects_all"]
        print("'nProcessing {dataset}")
        new_data = []
        for seq_dict in data:
            ...
            if self.use_identity:
                ...
                if subject_id in self.subjects_dict["train_subjects_all"]:
                    idx = self.subjects_dict["train_subjects_all"].index(subject_id)
                    one_hot = one_hot_labels[idx]
                else:
                    one_hot = one_hot_labels[0]
                    # ## set a subject as training set idx
                    # np.random.shuffle(possible_test_subjects)
                    # assigned_train_subj = possible_test_subjects[0]
                    # one_hot = one_hot_labels[(int(assigned_train_subj)-1)]
                    # print(f"******************* {dataset} {subject_id} ID is set with mdm_train ID {assigned_train_subj} ***********")
            else:
                one_hot = one_hot_labels[0]

            seq_len = (seq_len // 32) * 32  # to enable compression
        ...
        print("num of samples ignored", len(data)-len(new_data))
        return new_data
...
# def pose_processing(pose, normalize_pose, normalize_dim, to_6d_rotation):
#     if normalize_pose:
#         if normalize_dim == "xyz":
#             euler_pose = Rotation.from_rotvec(pose).as_euler("xyz", degrees=True)
#             simple_rot = euler_pose - euler_pose[:1]
#             norm_pose_aa = Rotation.from_euler("xyz", simple_rot, degrees=True).as_rotvec()
#             norm_pose = norm_pose_aa.astype(np.float32).reshape(-1, 3)
#         elif normalize_dim == "z" or normalize_dim == "y":
#             if normalize_dim == "y":
#                 order = "yzx"
#             elif normalize_dim == "z":
...
#             norm_z_pose = Rotation.from_euler(order, norm_z_pose, degrees=True).as_rotvec()
#             norm_z_pose = norm_z_pose.astype(np.float32).reshape(-1, 3)
#             norm_pose = norm_z_pose
#     else:
#         norm_pose = pose
#
#     if to_6d_rotation:
#         pose_6d = Rotation.from_rotvec(norm_pose).as_matrix().reshape(-1, 9)[:, :6]
#         norm_pose = pose_6d.astype(np.float32).reshape(-1, 6)
#
#     return norm_pose