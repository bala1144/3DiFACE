...
class TrainLoop:
    ...
    def set_attributes(self, args, train_platform, model, diffusion, data):
        ...
        if args.dataset == "vocaset" or "BIWI" or "biwi":
        ...
        if hasattr(args, 'schedule_sampler_type'):
        ...
        self.render_helper = render_helper(dataset=self.dataset)
    ...
    def visualize_during_train(self, diff_step, model_output, batch, x_t = None):
        ...
        print(audio.shape, motion.shape, identity.shape, template.shape, filename )
        ...
        if x_t is not None:
            curr_gt = x_t[0,:,:-1].detach().cpu().reshape(T, -1, 3) # T x 5023 x 3
